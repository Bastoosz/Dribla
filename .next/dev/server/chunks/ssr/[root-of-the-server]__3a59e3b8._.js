...
// --- FUNÇÃO HELPER: Calcula o status em tempo real (LÓGICA ATUALIZADA) ---
const getRealtimeStatus = (aluno, hoje)=>{
    // Se o BD diz que está pago, está em dia.
    if (aluno.status_mensalidade === 'pago') {
        return 'paga';
    }
    // Se chegou aqui, o status no BD é 'pendente'
    const vencimento = new Date(aluno.data_vencimento_mensalidade + 'T00:00:00'); // Garante data local
    vencimento.setHours(0, 0, 0, 0);
    // 1. Se a data de vencimento já passou
    if (vencimento < hoje) {
        return 'vencida';
    }
    // 2. Se a data de vencimento é hoje ou nos próximos 5 dias
    const cincoDias = new Date(hoje);
    cincoDias.setDate(hoje.getDate() + 5);
    if (vencimento >= hoje && vencimento <= cincoDias) {
        return 'proximo';
    }
    // 3. Se for 'pendente' mas a data de vencimento é futura (mais de 5 dias)
    // Para a UI, tratamos como 'Em Dia', pois não é 'proximo' nem 'vencida'
    return 'paga';
};
const ElencoPage = ()=>{
    const router = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$router$2e$js__$5b$ssr$5d$__$28$ecmascript$29$__["useRouter"])();
    ...
    // TODO: Buscar o limite real do treinador
    const [limiteAlunos, setLimiteAlunos] = (0, __TURBOPACK__imported__module__$5b$externals$5d2f$react__$5b$external$5d$__$28$react$2c$__cjs$29$__["useState"])(30);
...
//# sourceMappingURL=%5Broot-of-the-server%5D__3a59e3b8._.js.map